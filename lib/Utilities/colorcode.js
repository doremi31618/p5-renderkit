...
export class ColorCode{
  ...
  setRndColorObject(index){
    ...
    console.log('check origin unusecolorset', unuseColorSet)
    ...
    console.log('check origin colorset',this._colorUniqueSet)
    // console.log('unusecolorset', unuseColorSet,'rndIndex', rndIndex, "unuseColorSet", unuseColorIndex)
    let element = this.colorsetCollection[index].colorObject.id
    if (!this._colorUniqueSet.has(element))console.error('uniqueset does not have the element', this._colorUniqueSet);
    this._colorUniqueSet.delete(element);
    let color_object = ColorCode.ColorMap[unuseColorIndex];
    let new_element = color_object.id
    this._colorUniqueSet.add(new_element);
    console.log("index", index, 'check unique colorset', this._colorUniqueSet, "delete element", element, 'new element',new_element )
  ...
  static get ColorMap(){
    return [
      ...
      new ColorObject(8, 'yellow', [238, 210, 2]),
    ]
  } 
  ...
  static RegeneateColorCode(color_code_string){
    // console.log('Regenerate Color code', color_code_string);
    ...
    // console.log('check color set remain unique',this._colorUniqueSet )
  ...
  addColorset(colorset){
    // console.log('add colorset', colorset)
    ...
    this._colorUniqueSet.add(colorset.colorObject.id)
