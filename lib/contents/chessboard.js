...
export class cell{
  ...
  get centerPosX(){
    return this.coordinate_x + this.xSize/2;
  }
  get centerPosY(){
    return this.coordinate_y + this.ySize/2;
  }
  setXSize(size){
    this.xSize = size;
  }
...
export  class Chessboard extends Content{
  ...
  getCellPosByIndex(index){
    ...
    let newCell =  new cell(x_pos, y_pos, coordinate_x, coordinate_y)
    newCell.setXSize(this.xSize);
    newCell.setYSize(this.ySize);
