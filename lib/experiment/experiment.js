...
import {addRecord, setRecord} from '../Utilities/firebase.js';
import {addEvent, removeEvent} from '../Utilities/keyboard.js'
import { ExperimentSetting, instance } from "../Utilities/setting.js";

export class Experiment{
    constructor(session, associtate_id, subject_id, settingData=null){
        ...
        this.initialize(settingData);
    }
    initSession(session, associtate_id, subject_id){
        this.subject_id = subject_id || "example_subject_id";
        this.session = session || "example_session";
        this.associtate_id = associtate_id || "example_associate_id";
    }
    ...
    initialize(settingData){
        ...
        if (!settingData) settingData = ExperimentSetting.defaultSetting;
        
        //stage1 setting
        this.setting_version = settingData.version;
        this.setting_name = settingData.name
        this._stage1_min_level = settingData.min_level;
        this._stage1_max_level = settingData.max_level;
        this._max_tolerate_incorrect_number = settingData.max_tolerate_incorrect_number;
        this._stage1_assignment_of_each_level = settingData.number_of_each_level;
        
        //stage2 setting
        this._stage2_assignment_number = settingData.stage2_assignment_number;

        //stage3 setting
        this._stage3_assignment_number = settingData.stage3_assignment_number;

        //stage4 setting
        this._stage4_assignment_number = settingData.stage4_assignment_number;

        //generate stage1 answer
        ...
        console.log('check experiment init data', this, instance._data)
    }
    ...
    getExperimentData(){
        return{
            ...
            experience_config: {
                setting: instance._data,
                stage1_answers : this._stage1_assignment_answer,
                stage2_answers : this._stage2_assignment_answer,
                stage3_answers : this._stage3_assignment_answer,
                stage4_answers : this._stage4_assignment_answer,
            }

        }
    }
    async saveToFirebase(){
        let result_data = this.getExperimentData();
        let response = await setRecord("records",result_data);
    ...
    async _on_stage1_scene_end(){
        ...
        const stage1_failed = this.stage1_wrong_answer > this._max_tolerate_incorrect_number;
        ...
        if (this._trial+1 >= this._stage1_assignment_number || stage1_failed){
            ...
            try{
                ...
                console.log(e, 'upload file failed, auto download');
                this.downloadJSON(this.getExperimentData())
            }
    ...
    downloadJSON(obj, filename = 'experiment_record.json') {
        ...
        // Create an anchor element with the URL, and simulate a click to download
    ...
    async _on_stage4_scene_end(){
        ...
        if (stage4_trial+1 >= this._stage3_assignment_number){
            ...
            await this._getBreak("thank you for participating experiment", true);
