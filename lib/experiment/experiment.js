...
import {db, app, addData} from '../Utilities/firebase.js';
class AssignmentData{
    constructor(config){
        this.session = config.session || 'not-set';
        this.stage = config.stage || 'not-set';
        this.trial = config.trial || 'not-set';
        this.subject_id = config.subject_id || 'not-set';
        this.associate_id = config.associate_id || 'not-set';
        this.color_code_1 = config.color_code_1 || 'not-set';
        this.color_code_2 = config.color_code_2 || 'not-set';
        this.cue_index = config.cue_index || 'not-set';
        this.subject_response = config.subject_response || 'not-set';
        this.assignment_answer = config.assignment_answer || 'not-set';
        this.edit_time = config.edit_time || 'not-set';
    }
    toObject(){
        return{
            ...
            cue_index:this.cue_index,
            subject_response:this.subject_response,
...
export class Experiment{
    constructor(subject_id, session, associtate_id){
    ...
    initialize(){
        ...
        //stage2 setting
        this._stage2_assignment_number = 2;
        ...
        //stage3 setting
        this._stage3_assignment_number = 2;
        ...
        //stage4 setting
        this._stage4_assignment_number = 2;
    ...
    async saveRecord(_stage, response, answer){
        const record = {
            session: this.session,
            stage: _stage,
            trial: this._trial,
            ...
            edit_time: Date.now().toLocaleString()
        }
        await addData("records",record);

        //write to here
        // db.collection('ExperimentRecords').add(record)
        // .then((docRef)=>{
        ...
        // });
        this.records.push(new AssignmentData(record))
    }
    async _on_stage1_scene_end(){
        ...
        this.saveRecord(Experiment.Stage1, reponse.subject_response, traial_answer);


        this._trial += 1;
        if (this._trial % this._stage1_assignment_of_each_level == 0){
            this.current_level = 
        ...
        if (this._trial >= this._stage1_assignment_number || !isResponseCorrect){
            this.stage2_start_trial = this._trial;
            ...
            isAltColorCode = (this._stage1_assignment_answer.indexOf(this._trial) === -1)
    ...
    async _on_stage2_scene_end(){
        ...
        //step3 save trial record
        this.saveRecord(Experiment.Stage2, reponse.subject_response, traial_answer);

        //over stage1 max trial, switch to stage2
        let isAltColorCode,assignmentConfig;
        if (stage2_trial >= this._stage2_assignment_number){
            ...
            //general stage1 start
            isAltColorCode = (this._stage2_assignment_answer.indexOf(stage2_trial) === -1)
            assignmentConfig = this.generate_stage2_scene_config(isAltColorCode)
        }
    ...
    async _on_stage3_scene_end(){
        ...
        //step3 save trial record
        this.saveRecord(Experiment.Stage3, reponse.subject_response, traial_answer);

        //over stage1 max trial, switch to stage2
        let isAltColorCode,assignmentConfig;
        if (stage3_trial >= this._stage3_assignment_number){
            ...
            //general stage1 start
            isAltColorCode = (this._stage3_assignment_answer.indexOf(stage3_trial) === -1)
            assignmentConfig = this.generate_stage3_scene_config(isAltColorCode)
        }
    ...
    async _on_stage4_scene_end(){
        ...
        //step3 save trial record
        this.saveRecord(Experiment.Stage4, reponse.subject_response, traial_answer);

        //over stage1 max trial, switch to stage2
        let isAltColorCode,assignmentConfig;
        if (stage4_trial >= this._stage3_assignment_number){
            ...
            //general stage1 start
            isAltColorCode = (this._stage4_assignment_answer.indexOf(stage4_trial) === -1)
            assignmentConfig = this.generate_stage4_scene_config(isAltColorCode)
        }
